...
from .topology import DeltrelTopology, get_topology
...
def _static_value(components: Sequence[int], actor: int) -> float:
    ...
    cape_margin = components[actor * 6 + 1] - components[opponent * 6 + 1]
    value = (4.0 * total_margin + cape_margin) / 64.0
...
def _greedy_action_key(
    *,
    ...
    total_margin = components[actor * 6 + 5] - components[opponent * 6 + 5]
    cape_margin = components[actor * 6 + 1] - components[opponent * 6 + 1]
    shore_margin = components[actor * 6] - components[opponent * 6]
    ...
    return (
        ...
        friendly,
        enemy,
        int(topology.is_cape[action]),
        int(topology.is_shore[action]),
