"""Optional adapter for coarse ``deltrel_native.StateData`` buffers."""
...
class NativeStateDataProtocol(Protocol):
    """Structural subset exposed by ``deltrel_native.StateData``."""
...
class NativeVariantStateDataProtocol(NativeStateDataProtocol, Protocol):
    """Rules-v3 fields exposed by ``deltrel_native.StateData``."""
...
class NativeFeatureDataProtocol(Protocol):
    """Contiguous buffers exported by ``deltrel_native.FeatureData``."""
...
def load_deltrel_native(*, required: bool = False) -> ModuleType | None:
    """Import and fingerprint-check the extension only when requested."""

    try:
        module = importlib.import_module("deltrel_native")
    except ModuleNotFoundError as exc:
        if exc.name != "deltrel_native":
            raise
        if required:
            raise NativeCompatibilityError(
                "deltrel_native is not installed; build the optional PyO3 extension"
            ) from None
        return None
...
def validate_native_module(module: object) -> None:
    ...
    if not callable(hash_function):
        raise NativeCompatibilityError("deltrel_native lacks native_rules_hash()")
    ...
    if (
        ...
        raise NativeCompatibilityError(
            "deltrel_native rules hash does not match finalized TypeScript rules"
        )
    hash_tag = getattr(module, "native_rules_hash_tag", None)
    if callable(hash_tag) and hash_tag() != RULES_HASH_WIRE:
        raise NativeCompatibilityError("deltrel_native rules hash tag is incompatible")
    schema = getattr(module, "native_rules_schema", None)
    if callable(schema) and schema() != RULES_SCHEMA_ID:
        raise NativeCompatibilityError("deltrel_native rules schema is incompatible")
    ...
    if (
        ...
        raise NativeCompatibilityError(
            "deltrel_native feature schema does not match deltreltrain/features/v4; "
    ...
    if not callable(complete_clinches):
        raise NativeCompatibilityError(
            "deltrel_native lacks StateBatch.complete_clinches(); rebuild the native extension"
        )
    ...
    if not callable(search_algorithm) or search_algorithm() != SEARCH_ALGORITHM_ID:
        raise NativeCompatibilityError(
            "deltrel_native search algorithm is incompatible; rebuild the native extension"
        )
...
def positions_from_native(
    ...
    *,
    ...
    positions: list[DoubleDeltrelPosition] = []
    for row in range(batch_size):
...
def score_results_from_native(
    ...
    for row in range(batch_size):
        ...
        for player in range(2):
            ...
            players.append(
                PlayerScore(
                    shores=components[offset],
                    capes=components[offset + 1],
                    networks=components[offset + 2],
                    cape_bonus=components[offset + 3],
        ...
        results.append(
            ScoreResult(
                ...
                node_owner=owner,
                alive_stone=alive,
                contested_shores=components[base + 12],
                leader=leader,
            )
        )
    return results
