...
def _attach_routing_metadata(
    ...
    setattr(optimizer, "_deltreltrain_routing_metadata", metadata)
    return optimizer
...
def optimizer_routing_metadata(
    ...
    metadata = getattr(optimizer, "_deltreltrain_routing_metadata", None)
    if not isinstance(metadata, OptimizerRoutingMetadata):
        raise ValueError("optimizer has no DeltrelTrain routing metadata")
    if len(metadata.groups) != len(optimizer.param_groups):
