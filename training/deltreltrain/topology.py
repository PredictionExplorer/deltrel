"""Static Double Deltrel board topology, matching ``src/lib/deltrel/board.ts``."""
...
def get_topology(rings: int) -> DeltrelTopology:
    ...
    is_shore = torch.zeros(n, dtype=torch.bool)
    is_cape = torch.zeros(n, dtype=torch.bool)
    labels: list[str] = [""] * n
    ...
    relation_index = _relation_index(
        rings, ring_of, sector_of, pos_of, is_shore, shortest_path
    )
...
def _pair_relation_keys(
    ...
    shore_source = is_shore.to(torch.long).unsqueeze(1).expand(-1, is_shore.numel())
    shore_target = is_shore.to(torch.long).unsqueeze(0).expand(is_shore.numel(), -1)
    del rings
    return torch.stack(
        (ring_difference, angular_bucket, distance_bucket, shore_source, shore_target),
        dim=-1,
    )
...
def _validate_symmetry(topology: DeltrelTopology) -> None:
