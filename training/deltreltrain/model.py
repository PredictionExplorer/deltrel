...
(keyed by the pairwise relations of :mod:`deltreltrain.topology`) and adaLN-Zero
...
class DeltrelModelOutput(NamedTuple):
    ...
    final_shores_logits: Tensor | None = None
    final_networks_logits: Tensor | None = None
    final_capes_logits: Tensor | None = None
...
class GraphResTNet(nn.Module):
    ...
    def __init__(self, config: ModelConfig = ModelConfig()) -> None:
        ...
        if config.auxiliary_predictions:
            ...
            self.final_shores_head = nn.Linear(width, 2 * 51)
            self.final_networks_head = nn.Linear(width, 2 * 26)
            self.final_capes_head = nn.Linear(width, 2 * 6)
            for head in (
                self.opponent_reply_head,
                self.opponent_reply_swap_head,
                self.second_stone_head,
                self.final_shores_head,
                self.final_networks_head,
                self.final_capes_head,
            ):
    ...
    def forward(
        ...
        *,
        ...
        final_shores_logits = final_networks_logits = final_capes_logits = None
        if self.config.auxiliary_predictions and include_auxiliary:
            ...
            final_shores_logits = self.final_shores_head(pooled).reshape(
                batch_size, 2, 51
            )
            ...
            final_networks_logits = _mask_logits(
        ...
        return DeltrelModelOutput(
            ...
            final_shores_logits=final_shores_logits,
            final_networks_logits=final_networks_logits,
            final_capes_logits=final_capes_logits,
        )
