...
const FIXTURE_JSON: &str = include_str!("../../../../testdata/deltrel/conformance-v3.json");
...
fn finalized_v3_schema_hash_and_encodings_match_runtime_constants() {
    ...
    assert_eq!(fixture.rules.contract["variant"], "double-deltrel");
...
fn all_generated_board_and_d5_vectors_match() {
    ...
    for (&expected_rings, expected) in SUPPORTED_RINGS.iter().zip(&fixture.boards) {
        ...
        assert_eq!(board.shore_count(), expected.perimeter_count);
        ...
        for node in 0..board.node_count() {
            ...
            assert_eq!(
                u8::from(board.is_shore(node)),
                expected.perimeter_mask[index]
            );
            assert_eq!(u8::from(board.is_cape(node)), expected.cape_mask[index]);
            ...
            assert_eq!(expected_node.perimeter, board.is_shore(node));
            assert_eq!(expected_node.cape, board.is_cape(node));
...
fn assert_score(id: &str, actual: &ScoreResult, expected: &ScoreExpected, node_count: u16) {
    ...
    for (actual, expected) in actual.players.iter().zip(&expected.players) {
        assert_eq!(
            *actual,
            PlayerScore {
                shores: expected.shores,
                capes: expected.capes,
                networks: expected.networks,
                cape_bonus: expected.cape_bonus,
    ...
    assert_eq!(actual.contested_shores, expected.contested_shores, "{id}");
    assert_eq!(
