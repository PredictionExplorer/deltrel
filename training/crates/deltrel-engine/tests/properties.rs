...
fn assert_full_score_contract(rings: u8, score: &ScoreResult) -> TestCaseResult {
    ...
    prop_assert_eq!(score.contested_shores, 0);
...
proptest! {
    ...
    fn d5_and_color_swaps_preserve_the_scoring_contract(
        ...
        prop_assert_eq!(
            transformed_score.contested_shores,
            original_score.contested_shores
        );
        ...
        prop_assert_eq!(
            color_swapped.contested_shores,
            original_score.contested_shores
        );
        ...
        prop_assert_eq!(
            original_score.players[0].shores
                + original_score.players[1].shores
                + i16::try_from(original_score.contested_shores).unwrap(),
            i16::try_from(state.board().shore_count()).unwrap()
        );
