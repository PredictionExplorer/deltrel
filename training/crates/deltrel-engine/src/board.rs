...
impl Board {
    ...
    pub fn new(rings: u8) -> Result<Self, BoardError> {
        ...
        let mut shores = BitBoard::empty();
        let mut capes = BitBoard::empty();
    ...
    #[must_use]
    pub const fn shore_count(&self) -> u16 {
        self.shore_count
    }
    ...
    #[must_use]
    pub fn is_shore(&self, node: NodeId) -> bool {
        self.shores.contains(node)
    }

    /// Tests whether a node is one of the five corner capes.
    #[must_use]
    pub fn is_cape(&self, node: NodeId) -> bool {
        self.capes.contains(node)
    }
    ...
    #[must_use]
    pub const fn shore_mask(&self) -> BitBoard {
        self.shores
    }

    /// Fixed cape mask.
    #[must_use]
    pub const fn cape_mask(&self) -> BitBoard {
        self.capes
    }
    ...
    pub fn perimeter_cycle(&self) -> Vec<NodeId> {
        let mut nodes = Vec::with_capacity(usize::from(self.shore_count));
        for sector in 0..5 {
