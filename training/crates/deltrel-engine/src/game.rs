...
#[repr(u8)]
pub enum Mode {
    /// Classic Deltrel: one placement every turn.
    Classic = 0,
    /// Double Deltrel: two placements per turn after the opening.
    Double = 1,
}
...
impl Variant {
    /// Standard Double Deltrel: one opening stone, two per turn, no pie.
    ...
    /// Whether this is the standard no-pie Double Deltrel game.
    #[must_use]
...
impl fmt::Display for GameError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidTurnMetadata => f.write_str("invalid Deltrel turn metadata"),
...
/// Complete Deltrel state for every supported rule variant.
#[derive(Clone, Debug)]
...
impl GameState {
    /// Creates the empty standard Double Deltrel state for a prebuilt board.
    #[must_use]
