...
const FEATURE_SCHEMA_HASH: u64 = 0x058e_b071_d779_48a7;
const LEGACY_FEATURE_SCHEMA_VERSION: u8 = 3;
const LEGACY_FEATURE_SCHEMA_HASH: u64 = 0x4933_6c93_7d53_4865;
...
impl PyStateBatch {
    ...
    fn complete_exact_endgames(
        ...
        for (index, empty_nodes, last_move, turn_count, solved) in prepared {
            ...
            let winner = deltrel_engine::score_state(&solved.terminal)
                .leader
                .ok_or_else(|| {
    ...
    /// Exact static score, ownership, and alive-network annotations for all rows.
...
fn inference_state_key(state: &GameState, context: FeatureContext) -> Vec<u8> {
    ...
    bytes.extend_from_slice(b"deltrel-native-input-v1\0");
...
fn score_states(states: &[GameState], node_count: u16) -> PyScoreData {
    ...
    for (state, score) in states.iter().zip(&scores) {
        for player in score.players {
            components.extend([
                i32::from(player.shores),
                i32::from(player.capes),
                i32::from(player.networks),
                i32::from(player.cape_bonus),
                i32::from(player.award),
                i32::from(player.total),
            ]);
        }
        components.push(i32::from(score.contested_shores));
        ...
        let leader = if state.is_terminal() {
            Some(
                score
                    .leader
                    .expect("a full Deltrel board must have a decisive winner"),
            )
        } else {
            score.leader
        };
...
fn legacy_node_planes(
    ...
    let planes = [
        ...
        binary_feature(board.is_shore(node)),
        binary_feature(board.is_cape(node)),
...
fn legacy_global_scalars(
    ...
    let deltrel_scale = (f64::from(board.shore_count()) / 2.0).max(1.0);
    [
        ...
        (f64::from(current_score.shores) / f64::from(board.shore_count())) as f32,
        (f64::from(opponent_score.shores) / f64::from(board.shore_count())) as f32,
        (f64::from(current_score.capes) / 5.0) as f32,
        (f64::from(opponent_score.capes) / 5.0) as f32,
        (f64::from(current_score.networks) / deltrel_scale) as f32,
        (f64::from(opponent_score.networks) / deltrel_scale) as f32,
        (f64::from(score.contested_shores) / f64::from(board.shore_count())) as f32,
    ]
}
...
fn packed_score_components(score: &ScoreResult) -> [i32; SCORE_COMPONENT_DIM] {
    ...
    [
        i32::from(zero.shores),
        i32::from(zero.capes),
        i32::from(zero.networks),
        i32::from(zero.cape_bonus),
        i32::from(zero.award),
        i32::from(zero.total),
        i32::from(one.shores),
        i32::from(one.capes),
        i32::from(one.networks),
        i32::from(one.cape_bonus),
        i32::from(one.award),
        i32::from(one.total),
        i32::from(score.contested_shores),
...
const fn native_search_algorithm_id() -> &'static str {
    deltrel_search::SEARCH_ALGORITHM_ID
}
...
fn configure_rayon_threads(threads: usize) -> PyResult<()> {
    ...
    ThreadPoolBuilder::new()
        .num_threads(threads)
        .thread_name(|index| format!("deltrel-actor-{index}"))
        .build_global()
        .map_err(|error| {
...
#[pymodule]
fn deltrel_native(module: &Bound<'_, PyModule>) -> PyResult<()> {
...
mod tests {
    ...
    fn feature_v4_encodes_variants_history_and_context() {
        ...
        assert_eq!(features.feature_schema_hash(), 0x058e_b071_d779_48a7);
    ...
    fn legacy_feature_v3_is_still_produced_from_the_four_field_metadata() {
        ...
        assert_eq!(features.feature_schema_hash(), 0x4933_6c93_7d53_4865);
        assert_eq!(
