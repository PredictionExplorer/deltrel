            ...
            try:
                ...
                if not (
                    ...
                    if manifest.role != "champion":
                        raise ValueError(
                            "deltrelserve accepts only a champion model pointer"
                        )
...
class NativeAnalysisService:
    def __init__(
        ...
        *,
        ...
        self.config = config
        self.native: Any = native_module or load_deltrel_native(required=True)
    ...
    def _import_state(self, request: AnalyzeRequest) -> Any:
        ...
        if not callable(importer):
            raise AnalysisError(
                "native_incompatible",
                "deltrel_native lacks StateBatch.from_semantic",
            )
        node_count = len(request.stones)
...
def _auxiliary_payload(
    ...
    *,
    ...
    pairs = (
        prediction.final_shores,
        prediction.final_networks,
        prediction.final_capes,
        prediction.cape_bonus_probability,
    )
    ...
    for player in (0, 1):
        ...
        counts.append(
            {
                "player": player,
                "shores": min(float(maxima[0]), prediction.final_shores[index]),
                "networks": min(float(maxima[1]), prediction.final_networks[index]),
                "corners": min(5.0, prediction.final_capes[index]),
                "corner_bonus_probability": min(
                    1.0, prediction.cape_bonus_probability[index]
                ),
            }
        )
