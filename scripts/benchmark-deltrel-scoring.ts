...
import { getBoard } from '../src/lib/deltrel/board';
import { scorePosition } from '../src/lib/deltrel/scoring';

const rings = Number(process.env.DELTREL_BENCH_RINGS ?? 10);
const iterations = Number(process.env.DELTREL_BENCH_ITERATIONS ?? 50_000);
const maximumMicros = Number(process.env.DELTREL_BENCH_MAX_US ?? Number.POSITIVE_INFINITY);
if (
  ...
  throw new Error('invalid DELTREL_BENCH_RINGS, DELTREL_BENCH_ITERATIONS, or DELTREL_BENCH_MAX_US');
}
...
const result = {
  schemaVersion: 1,
  benchmark: 'typescript-deltrel-scoring',
  rings,
  nodes: board.n,
  iterations,
  elapsedMs,
