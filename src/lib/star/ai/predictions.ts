...
import type { StarAiRequest } from './protocol';
...
export interface StarAiPredictions {
  perspective: 0 | 1;
...
function invalid(): never {
  throw new StarAiError('protocol', 'AI future predictions are invalid.');
}
...
export function parsePredictions(value: unknown): StarAiPredictions | null {
  if (value === null) return null;
  ...
  const finalCounts = value.finalCounts.map((item, index): FinalPlayerPrediction => {
    if (!record(item) || !exact(item, ['player', 'peries', 'stars', 'corners', 'cornerBonusProbability']) ||
      item.player !== index || (item.player !== 0 && item.player !== 1) ||
      !bounded(item.peries, 50) || !bounded(item.stars, 25) || !bounded(item.corners, 5) ||
      !bounded(item.cornerBonusProbability, 1)) return invalid();
    return {
      player: item.player, peries: item.peries, stars: item.stars,
...
export function parseServerPredictions(value: unknown): StarAiPredictions | null {
  if (value === null) return null;
  ...
  return parsePredictions({
    ...
    finalCounts: value.final_counts.map((item) => {
      if (!record(item) || !exact(item, ['player', 'peries', 'stars', 'corners', 'corner_bonus_probability'])) return invalid();
      return {
        player: item.player, peries: item.peries, stars: item.stars,
...
export function validatePredictionState(predictions: StarAiPredictions, request: StarAiRequest): void {
  const state = request.state;
  if (predictions.perspective !== state.toMove || predictions.finalCounts.some(
    (item) => item.peries > 5 * state.rings || item.stars > Math.floor(5 * state.rings / 2),
  )) return invalid();
