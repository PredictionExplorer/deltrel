...
import {
  createDeltrelConformance,
  serializeDeltrelConformance,
} from '../conformance';
import {
  fnv1a64,
  DELTREL_ACTION_LAYOUT_SCHEMA_ID,
  DELTREL_CONFORMANCE_SCHEMA_ID,
...
describe('versioned rules contract', () => {
  it('pins the complete v3 gameplay contract', () => {
    expect(DELTREL_RULES_VERSION).toBe(3);
    expect(DELTREL_RULES_SCHEMA_ID).toBe('deltrel.rules.v3');
    expect(DELTREL_RULES_HASH_ALGORITHM).toBe('fnv1a64');
    expect(`${DELTREL_RULES_HASH_ALGORITHM}:${fnv1a64(DELTREL_RULES_CANONICAL)}`).toBe(
      DELTREL_RULES_HASH,
    );
    expect(DELTREL_RULES_HASH).toBe(EXPECTED_RULES_HASH);
    ...
    for (const requiredClause of [
      ...
      expect(DELTREL_RULES_CANONICAL).toContain(requiredClause);
    }
  });

  it('keeps model features and node-only layout separately versioned', () => {
    expect(DELTREL_FEATURE_SCHEMA_ID).toBe(
      'deltrel.model-features.external.v3',
    );
    expect(DELTREL_ACTION_LAYOUT_SCHEMA_ID).toBe(
      'deltrel.action-layout.nodes-only.v1',
    );
    expect(DELTREL_RULES_CANONICAL).not.toContain(DELTREL_FEATURE_SCHEMA_ID);
    expect(DELTREL_RULES_CANONICAL).not.toContain(DELTREL_ACTION_LAYOUT_SCHEMA_ID);
  });
});
...
describe('v3 conformance export', () => {
    ...
    const numericSemantics = (value: unknown): unknown => {
      ...
      if (value !== null && typeof value === 'object') {
        return Object.entries(value)
          ...
          .map(([, item]) => numericSemantics(item));
      }
      return typeof value === 'string' ? null : value;
    };
    // Recorded before the identity/coordinate change. Covers every fixture
    // graph, symmetry, score, atomic action, full game, and turn transition.
    expect(sha256(JSON.stringify(numericSemantics(createDeltrelConformance())))).toBe(
      'sha256:4c4129690ca2390713c454c73acbef7c561da8dc7a2a9b536fbe377b4fbd60db',
    );
  });

  it('is deterministic and contains no legacy action or outcome fields', () => {
    const first = serializeDeltrelConformance(createDeltrelConformance());
    const second = serializeDeltrelConformance(createDeltrelConformance());
  ...
  it('exports exact topology and D5 maps only for supported rings', () => {
    ...
    expect(conformance.schemas).toEqual({
      ...
      modelFeatures: DELTREL_FEATURE_SCHEMA_ID,
    ...
    for (const board of conformance.boards) {
      ...
      expect(board.capeMask).toHaveLength(board.nodeCount);
  ...
  it('contains full-board traces with binary terminal outcomes', () => {
    ...
    for (const game of conformance.games) {
      ...
      expect(game.terminal.score.contestedShores).toBe(0);
      expect(
  ...
  it('matches the checked-in deterministic v3 fixture', () => {
    const fixture = readFileSync(
      new URL('../../../../testdata/deltrel/conformance-v3.json', import.meta.url),
      'utf8',
    );
    expect(fixture).toBe(serializeDeltrelConformance(createDeltrelConformance()));
  });
});
