 ...
 *
 * Implements the same scoring semantics as src/lib/deltrel/scoring.ts but with a
...
export function referenceScore(board: Board, stones: ArrayLike<number>): RefResult {
  ...
  // Official static network test: survival depends only on perimeter nodes the
  ...
  const alive = groups.map(
    (group) => group.cells.filter((u) => board.isShore[u]).length >= 2,
  );
  ...
  for (let gi = 0; gi < groups.length; gi++) {
    ...
    networks[groups[gi].color]++;
    for (const u of groups[gi].cells) {
      aliveStone[u] = 1;
      nodeOwner[u] = groups[gi].color;
      if (board.isShore[u]) {
        shores[groups[gi].color]++;
        if (board.isCape[u]) capes[groups[gi].color]++;
      }
    }
  }
  for (const region of regions) {
    ...
    for (const u of region.cells) {
      nodeOwner[u] = owner;
      if (board.isShore[u]) {
        if (owner === -1) contestedShores++;
        else {
          shores[owner]++;
          if (board.isCape[u]) capes[owner]++;
        }
      }
    }
  }
  ...
  return { players, nodeOwner, aliveStone, contestedShores };
}
