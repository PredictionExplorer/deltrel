    ...
    for (const rings of SUPPORTED_RINGS) {
      ...
      for (const symmetry of D5_SYMMETRIES) {
        ...
        for (let u = 0; u < board.n; u++) {
          ...
          expect(board.isShore[mapped]).toBe(board.isShore[u]);
          expect(board.isCape[mapped]).toBe(board.isCape[u]);
    ...
    for (const rings of SUPPORTED_RINGS) {
      ...
      for (const density of [0.08, 0.45, 0.9]) {
        ...
        for (const symmetry of D5_SYMMETRIES) {
          ...
          expect(transformed.contestedShores).toBe(
            original.contestedShores,
          );
...
describe('D5 Double Deltrel transition and replay equivariance', () => {
