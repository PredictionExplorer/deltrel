/**
 * Game state for Deltrel, modeled as a pure reducer over an action log.
 *
 * Modes:
 *   - classic:  one stone per turn.
 *   - double:   Double Deltrel — two stones per turn, except the very first
...
function validateConfig(config: GameConfig): void {
  ...
  if (
    ...
    throw new Error(`handicap must be an integer in 1..${DELTREL_MAX_HANDICAP}`);
  }
