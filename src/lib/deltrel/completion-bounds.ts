...
export interface CompletionBounds {
  ...
  /**
   * Existing stones that cannot belong to a living network in any completion.
 ...
 *
 *   total0 - capeBonus0
 *     = sum_0 max(componentShores - 2, 0)
 *     + sum_1 min(componentShores, 2)
 *
 ...
 * and owned capes cannot move away from the player receiving the node.
