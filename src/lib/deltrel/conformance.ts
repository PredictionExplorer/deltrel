 ...
 * Games cover every rule variant: standard Double Deltrel, classic one-stone
...
function boardVector(board: Board) {
  ...
  return {
    rings: board.rings,
    nodeCount: board.n,
    perimeterCount: board.shoreCount,
    ...
    perimeterMask: Array.from(board.isShore),
    capeMask: Array.from(board.isCape),
    labels: [...board.labels],
    ...
    nodes: Array.from({ length: board.n }, (_, id) => ({
      ...
      perimeter: board.isShore[id] === 1,
      cape: board.isCape[id] === 1,
...
function scoreExpectation(score: ScoreResult) {
  return {
    ...
    contestedShores: score.contestedShores,
    leader: score.leader,
  };
}
...
export function createDeltrelConformance() {
  ...
  for (const rings of SUPPORTED_RINGS) {
    ...
    scores.push(
      scoreVector(`rings-${rings}-two-perimeter-deltrel`, board, twoPerimeter),
    );
    scores.push(
      scoreVector(
        `rings-${rings}-sparse`,
  ...
  return {
    ...
    rules: {
      ...
      hashAlgorithm: DELTREL_RULES_HASH_ALGORITHM,
      hash: DELTREL_RULES_HASH,
      canonical: DELTREL_RULES_CANONICAL,
      contract: DELTREL_RULES_CONTRACT,
    },
    outcomeEncoding: {
      loss: 0,
      win: 1,
    ...
    actionLayouts: {
      schema: DELTREL_ACTION_LAYOUT_SCHEMA_ID,
      modelFeatureSchema: DELTREL_FEATURE_SCHEMA_ID,
...
export type DeltrelConformance = ReturnType<typeof createDeltrelConformance>;

/** Stable pretty-printed representation used for checked-in test data. */
export function serializeDeltrelConformance(
  conformance: DeltrelConformance = createDeltrelConformance(),
): string {
