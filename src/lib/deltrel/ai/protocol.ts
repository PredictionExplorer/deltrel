...
export const DELTREL_FEATURE_SCHEMA_VERSION = 4 as const;
export const DELTREL_ACTION_LAYOUT_VERSION = 1 as const;

/** Exact canonical feature contract from training/deltreltrain/contracts.py. */
export const DELTREL_FEATURE_CONTRACT = [
  'deltreltrain/features/v4;',
  ...
  'alive-current,alive-opponent,shore,cape,ring-fraction,arm-distance,',
  ...
  'opening,terminal,current-score,opponent-score,margin,current-shores,',
  'opponent-shores,current-capes,opponent-capes,current-networks,',
  'opponent-networks,contested-shores,turn-size,handicap,handicap-phase,',
  'handicap-remaining,pie-pending,swap-available,history-known,pda;',
  'edges=tangential,radial-diagonal,bridge;',
  'relations=ring-difference,angular-offset-bucket,shortest-path-bucket,shore-pair;',
  'sample-actions=node[0:N];',
  'batch-actions=node[0:maxN];',
  'soft-policy=katago-temperature-4',
].join('');

export const DELTREL_FEATURE_SCHEMA_HASH = fnv1a64(DELTREL_FEATURE_CONTRACT);

/** Atomic actions: one placement or the pie swap. */
export type AtomicGameAction = GameAction;

export interface DeltrelAiPlacementHistory {
...
export interface DeltrelAiSemanticState {
  rings: number;
  ...
  swapped: boolean;
  history: DeltrelAiPlacementHistory;
}

export interface DeltrelAiRequest {
...
export interface DeltrelAiResponse {
  schema: typeof DELTREL_AI_PROTOCOL_SCHEMA_ID;
  version: typeof DELTREL_AI_PROTOCOL_VERSION;
  requestId: string;
  rulesHash: typeof DELTREL_RULES_HASH;
...
export type AiResponseAcceptance =
  | { ok: true; action: AtomicGameAction; response: DeltrelAiResponse }
...
export function newAiRequestId(): string {
  ...
  requestSequence += 1;
  return `deltrel-ai-${Date.now().toString(36)}-${requestSequence.toString(36)}`;
}
...
export function codeToAction(code: number, nodeCount: number): AtomicGameAction {
  ...
  throw new DeltrelAiError('protocol', `Invalid atomic action code: ${String(code)}.`);
}

export function semanticStateFromGame(state: GameState): DeltrelAiSemanticState {
  return {
...
export function isPiePending(state: DeltrelAiSemanticState): boolean {
  return state.pie && state.opening && state.movesLeft === state.handicap;
}

/**
 * Mirrors deltrel_engine::GameState::hash64 so browser requests can be checked
 * against the replayed WASM state without trusting presentation metadata.
 */
export function semanticStateHash(state: DeltrelAiSemanticState): string {
  if (typeof BigInt !== 'function') {
    throw new DeltrelAiError('unavailable', 'AI controllers require BigInt browser support.');
  }
...
export function validateAiConfig(config: GameConfig): void {
  ...
  if (
    ...
    throw new DeltrelAiError('protocol', 'AI controllers received an invalid rule variant.');
  }
}
...
export function buildAiRequest(
  ...
  if (game.over) {
    throw new DeltrelAiError('protocol', 'Cannot request an action for a terminal position.');
  }
  const nodeCount = game.board.n;
  const actionLog = log.map((action) => {
    if (action.type !== 'place' && action.type !== 'swap') {
      throw new DeltrelAiError('protocol', 'AI action logs contain placements and swaps only.');
    }
...
function parseAction(rawAction: unknown): AtomicGameAction {
  if (!isRecord(rawAction)) {
    throw new DeltrelAiError('protocol', 'AI response must contain one atomic action.');
  }
  if (rawAction.type === 'swap') {
    if (!hasExactKeys(rawAction, ['type'])) {
      throw new DeltrelAiError('protocol', 'A swap action contains unknown fields.');
    }
    return { type: 'swap' };
  }
  ...
  if (
    ...
    throw new DeltrelAiError('protocol', 'A placement must contain a non-negative node id.');
  }
  if (!hasExactKeys(rawAction, ['type', 'node'])) {
    throw new DeltrelAiError('protocol', 'A placement action contains unknown fields.');
  }
...
export function parseAiResponse(request: DeltrelAiRequest, payload: unknown): DeltrelAiResponse {
  if (!isRecord(payload)) {
    throw new DeltrelAiError('protocol', 'AI response must be an object.');
  }
  if (
    payload.schema !== DELTREL_AI_PROTOCOL_SCHEMA_ID ||
    payload.version !== DELTREL_AI_PROTOCOL_VERSION ||
    payload.rulesHash !== DELTREL_RULES_HASH
  ) {
    throw new DeltrelAiError('protocol', 'AI response schema or rules hash is incompatible.');
  }
  if (payload.requestId !== request.requestId || payload.stateHash !== request.stateHash) {
    throw new DeltrelAiError('stale', 'AI response belongs to an obsolete position.');
  }
  if ('actions' in payload) {
    throw new DeltrelAiError('protocol', 'AI response must not contain a multi-action turn.');
  }
  if (
    ...
    throw new DeltrelAiError('protocol', 'AI response contains unknown fields.');
  }

  const action = parseAction(payload.action);
  if (!isRequestLegalAction(request, action)) {
    throw new DeltrelAiError('illegal', 'AI returned an illegal atomic action.');
  }
...
export function acceptAiResponse(
  ...
  let response: DeltrelAiResponse;
  try {
    response = parseAiResponse(request, payload);
  } catch (error) {
    if (error instanceof DeltrelAiError) {
      const code =
