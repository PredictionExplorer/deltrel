...
import { DeltrelAiError, type DeltrelAiErrorCode } from '../errors';
import {
  DELTREL_AI_PROTOCOL_SCHEMA_ID,
  DELTREL_AI_PROTOCOL_VERSION,
  DELTREL_FEATURE_CONTRACT,
...
function expectAiError(
  ...
  try {
    operation();
    throw new Error('expected DeltrelAiError');
  } catch (error) {
    expect(error).toBeInstanceOf(DeltrelAiError);
...
describe('atomic AI protocol v3', () => {
  it('builds the exact opening semantic request', () => {
    ...
    expect(DELTREL_AI_PROTOCOL_VERSION).toBe(3);
    expect(DELTREL_FEATURE_SCHEMA_VERSION).toBe(4);
    expect(DELTREL_FEATURE_SCHEMA_HASH).toBe(fnv1a64(DELTREL_FEATURE_CONTRACT));
    expect(DELTREL_FEATURE_SCHEMA_HASH).toBe('058eb071d77948a7');
  ...
  it('strictly rejects malformed response shapes and identities', () => {
    ...
    const valid = makeAiResponse(request, { type: 'place', node: 0 });
    const cases: Array<[unknown, DeltrelAiErrorCode, string]> = [
