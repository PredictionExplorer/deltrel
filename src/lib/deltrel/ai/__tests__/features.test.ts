...
import {
  ...
  DELTREL_GLOBAL_FEATURE_NAMES,
  DELTREL_NODE_FEATURE_DIM,
  DELTREL_NODE_FEATURE_NAMES,
...
describe('schema-v3 browser features', () => {
  it('uses exact feature dimensions and a nodes-only action layout', () => {
    ...
    const encoded = encodeDeltrelFeatures(state);
    const board = getBoard(4);

    expect(DELTREL_NODE_FEATURE_DIM).toBe(19);
    expect(DELTREL_GLOBAL_FEATURE_DIM).toBe(25);
  ...
  it('normalizes terminal score support by 151 and masks every action', () => {
    const board = getBoard(4);
    const encoded = encodeDeltrelFeatures({
      rings: 4,
...
describe('cross-language feature parity', () => {
  it('reproduces every Python schema-v4 vector of the checked-in fixture', () => {
    const fixture = JSON.parse(
      readFileSync(
        new URL('../../../../../testdata/deltrel/features-v4.json', import.meta.url),
        'utf8',
      ),
    ) as {
    ...
    expect(fixture.featureSchemaVersion).toBe(DELTREL_FEATURE_SCHEMA_VERSION);
    expect(fixture.featureSchemaHash).toBe(DELTREL_FEATURE_SCHEMA_HASH);
    expect(fixture.rulesHash).toBe(DELTREL_RULES_HASH);
    expect(fixture.nodeFeatureNames).toEqual([...DELTREL_NODE_FEATURE_NAMES]);
    expect(fixture.globalFeatureNames).toEqual([...DELTREL_GLOBAL_FEATURE_NAMES]);
