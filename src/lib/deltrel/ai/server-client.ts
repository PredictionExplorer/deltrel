...
export const DELTRELSERVE_API_SCHEMA_VERSION = 3 as const;

export interface AnalyzeRequestV3 {
  schema_version: typeof DELTRELSERVE_API_SCHEMA_VERSION;
  rules_hash: DeltrelAiRequest['rulesHash'];
  rings: number;
  stones: number[];
  to_move: 0 | 1;
  moves_left: number;
...
function strictBudgetInteger(name: string, value: unknown, maximum: number): number {
  if (
    ...
    throw new DeltrelAiError('protocol', `${name} must be an integer in 1..${maximum}.`);
  }
  return value;
}
...
export function resolveServerSearchBudget(
  ...
  publicValues: { simulations?: string; maxConsidered?: string } = {
    simulations: process.env.NEXT_PUBLIC_DELTREL_AI_SIMULATIONS,
    maxConsidered: process.env.NEXT_PUBLIC_DELTREL_AI_MAX_CONSIDERED,
  },
): ServerSearchBudget {
  const configured = {
...
export function toAnalyzeRequest(
  ...
  if (request.state.terminal) {
    throw new DeltrelAiError('protocol', 'Deltrelserve accepts only active positions.');
  }
  if (semanticStateHash(request.state) !== request.stateHash) {
    throw new DeltrelAiError('protocol', 'Internal AI state hash is inconsistent.');
  }
  return {
    schema_version: DELTRELSERVE_API_SCHEMA_VERSION,
...
/**
 * NEXT_PUBLIC_DELTREL_AI_URL may be the full /v2/move URL or a deployment base.
...
export function resolveDeltrelAiHealthUrl(value: string): string {
  const moveUrl = resolveDeltrelAiMoveUrl(value);
...
export function configuredServerAiUrl(): string {
  const value = process.env.NEXT_PUBLIC_DELTREL_AI_URL;
  return value?.trim() ? resolveDeltrelAiMoveUrl(value) : DEFAULT_DELTREL_AI_MOVE_URL;
}

export function configuredServerHealthUrl(): string {
  const value = process.env.NEXT_PUBLIC_DELTREL_AI_URL;
  return value?.trim() ? resolveDeltrelAiHealthUrl(value) : DEFAULT_DELTREL_AI_HEALTH_URL;
}
...
function parseAtomicAction(value: unknown, label: string): ParsedAtomicAction {
  if (
    ...
    throw new DeltrelAiError('protocol', `${label} is not a valid atomic action.`);
  }
  if (
    value.kind === 'place' &&
    value.code >= 0 &&
  ...
  throw new DeltrelAiError('protocol', `${label} code, kind, and node disagree.`);
}
...
function parseFiniteArray(
  ...
  if (
    ...
    throw new DeltrelAiError('protocol', `${label} is invalid.`);
  }
...
export function parseAnalyzeResponse(
  ...
  if (
    ...
    throw new DeltrelAiError('stale', 'Deltrelserve response identity is incompatible.');
  }

  const action = parseAtomicAction(payload.action, 'Deltrelserve action');
  if (!request.legalActions.includes(action.code)) {
    throw new DeltrelAiError('illegal', 'Deltrelserve selected an illegal action.');
  }
  if (!Array.isArray(payload.root_actions) || payload.root_actions.length === 0) {
    throw new DeltrelAiError('protocol', 'Deltrelserve root actions are invalid.');
  }
  const rootActions = payload.root_actions.map((item, index) =>
    parseAtomicAction(item, `Deltrelserve root action ${index}`),
  );
  ...
  if (
    ...
    throw new DeltrelAiError('protocol', 'Deltrelserve root action identity is inconsistent.');
  }

  const rootPolicy = parseFiniteArray(
    payload.root_policy,
    'Deltrelserve root policy',
    rootActions.length,
    (item) => item >= 0,
  );
  const rootQ = parseFiniteArray(
    payload.root_q,
    'Deltrelserve root Q values',
    rootActions.length,
    (item) => item >= -1 && item <= 1,
  );
  if (
    ...
    throw new DeltrelAiError('protocol', 'Deltrelserve root statistics are invalid.');
  }
  ...
  if (
    ...
    throw new DeltrelAiError('protocol', 'Deltrelserve outcome belief is invalid.');
  }
  ...
  if (
    ...
    throw new DeltrelAiError('protocol', 'Deltrelserve value belief is invalid.');
  }
  ...
  if (
    ...
    throw new DeltrelAiError('protocol', 'Deltrelserve variant metadata is incompatible.');
  }
  ...
  if (
    ...
    throw new DeltrelAiError('protocol', 'Deltrelserve score belief is invalid.');
  }
  const scoreProbabilities = parseFiniteArray(
    payload.score_belief.probabilities,
    'Deltrelserve score probabilities',
    303,
    (item) => item >= 0,
  );
  if (!approximatelyOne(scoreProbabilities, 1e-5)) {
    throw new DeltrelAiError('protocol', 'Deltrelserve score probabilities are not normalized.');
  }
  ...
  if (
    ...
    throw new DeltrelAiError('protocol', 'Deltrelserve expected score margin is inconsistent.');
  }
  ...
  if (
    ...
    throw new DeltrelAiError('protocol', 'Deltrelserve model or timing metadata is invalid.');
  }
...
export async function requestServerAiDecision(
  ...
  const url = options.url?.trim()
    ? resolveDeltrelAiMoveUrl(options.url)
    : configuredServerAiUrl();

  const timeoutMs = options.timeoutMs ?? DEFAULT_DELTREL_AI_TIMEOUT_MS;
  if (!Number.isFinite(timeoutMs) || timeoutMs <= 0) {
    throw new DeltrelAiError('protocol', 'AI timeout must be a positive number.');
  }
  if (options.signal?.aborted) {
    throw new DeltrelAiError('cancelled', 'AI request cancelled.');
  }
...
export async function requestServerAiAction(
  ...
  return responseFromDeltrelAiDecision(await requestServerAiDecision(request, options));
}
