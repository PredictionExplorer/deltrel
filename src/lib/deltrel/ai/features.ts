...
import { isPiePending, type DeltrelAiSemanticState } from './protocol';

/** Playout-doubling advantage input range; browser games are symmetric (0). */
export const DELTREL_MAX_PLAYOUT_DOUBLING_ADVANTAGE = 3;

export const DELTREL_NODE_FEATURE_NAMES = [
  'empty',
  'current_stone',
  'opponent_stone',
  'owner_current',
...
export const DELTREL_GLOBAL_FEATURE_NAMES = [
  'rings_fraction',
  'occupancy_fraction',
...
export const DELTREL_MODEL_INPUT_NAMES = [
  'node_features',
  'global_features',
  'neighbor_index',
  'neighbor_mask',
...
export const DELTREL_MODEL_OUTPUT_NAMES = [
  'policy_logits',
  'outcome_logits',
  'score_margin_logits',
  'ownership_logits',
...
export const DELTREL_NODE_FEATURE_DIM = DELTREL_NODE_FEATURE_NAMES.length;
export const DELTREL_GLOBAL_FEATURE_DIM = DELTREL_GLOBAL_FEATURE_NAMES.length;
...
export interface EncodedDeltrelFeatures {
  nodeCount: number;
  maxDegree: number;
...
function turnSize(state: DeltrelAiSemanticState): number {
  return state.mode === 'classic' ? 1 : 2;
}

/** Placements of the turn in progress: the handicap during the opening. */
function currentTurnTotal(state: DeltrelAiSemanticState): number {
...
function validateSemanticState(state: DeltrelAiSemanticState): Board {
  ...
  if (
    ...
    state.handicap < 1 ||
    state.handicap > DELTREL_MAX_HANDICAP
  ) {
    throw new Error(`handicap must be in 1..${DELTREL_MAX_HANDICAP}`);
  }
...
/**
 * Exact browser port of training/deltreltrain/features.py schema v4 with the
 * browser's fixed context: history is always known and no side holds a
 * playout-doubling advantage.
 */
export function encodeDeltrelFeatures(state: DeltrelAiSemanticState): EncodedDeltrelFeatures {
  ...
  const nodeFeatures = new Float32Array(board.n * DELTREL_NODE_FEATURE_DIM);
  ...
  for (let node = 0; node < board.n; node++) {
    ...
    const base = node * DELTREL_NODE_FEATURE_DIM;
    const values = [
      bool(empty),
      bool(currentStone),
      bool(opponentStone),
      ...
      bool(Boolean(board.isShore[node])),
      bool(Boolean(board.isCape[node])),
      ring / state.rings,
  ...
  const networkScale = Math.max(1, board.shoreCount / 2);
  ...
  const globalFeatures = new Float32Array([
    ...
    currentScore.shores / board.shoreCount,
    opponentScore.shores / board.shoreCount,
    currentScore.capes / 5,
    opponentScore.capes / 5,
    currentScore.networks / networkScale,
    opponentScore.networks / networkScale,
    score.contestedShores / board.shoreCount,
    turnSize(state) / 2,
    state.handicap / DELTREL_MAX_HANDICAP,
    bool(state.opening && state.handicap >= 2),
    state.opening ? state.movesLeft / DELTREL_MAX_HANDICAP : 0,
    bool(isPiePending(state)),
    bool(state.swapAvailable),
    1,
    0 / DELTREL_MAX_PLAYOUT_DOUBLING_ADVANTAGE,
  ]);
