...
test('starts a named human game and places a stone with the mouse', async ({ page }) => {
  ...
  await expect(
    page.getByRole('button', {
      name: /Node A10, Ada stone on interior node, last move/i,
    }),
...
test('undoes and redoes a placement through the visible controls', async ({ page }) => {
  await startMiniHumanGame(page);
  await page
    .getByRole('button', { name: /Node A10, empty interior node/i })
    .click();
  ...
  await undo.click();
  await expect(
    page.getByRole('button', { name: /Node A10, empty interior node/i }),
  ).toBeVisible();
  ...
  await redo.click();
  await expect(
    page.getByRole('button', { name: /Node A10, Ada stone on interior node/i }),
  ).toBeVisible();
  await expect(undo).toBeEnabled();
  await expect(redo).toBeDisabled();
});
...
test('branches from a reviewed position with play-from-here', async ({ page }) => {
  await startMiniHumanGame(page);
  for (const label of ['A10', 'B10', 'C10'] as const) {
    await page
      .getByRole('button', {
